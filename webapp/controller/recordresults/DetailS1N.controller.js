sap.ui.define([
	...
	return Controller.extend("de.enercon.usbee.controller.recordresults.DetailS1N", {

		onInit: function () {
			this.getView().setModel(this.getOwnerComponent().getModel("GLOBAL"), "global");

			var oRouter = this.getOwnerComponent().getRouter();
			oRouter.getRoute("Merkmal_S1_Quan").attachMatched(this._onRouteMatched, this);
		...
		// 		"',Prueflos_Key_Object='" + oArgs.pl_key_object + "',Vornr='" + oArgs.vornr + "',Pruefpunkt='" + oArgs.pruefpunkt + "',Merknr='" + oArgs.merknr + "')";
		// 	var path1 = "pl>" + path;
		...
		// 				// var oTOLS = new JSONModel({
		// 				// 	_TOL_OBEN: oEvent.getParameter("data").UpTolLmt.replace(/\s+/g, ''),
		// 				// 	_TOL_UNTEN: oEvent.getParameter("data").LwTolLmt.replace(/\s+/g, '')
		// 				// });
		// 				// oView.setModel(oTOLS, "TOLS");
		// 				// Das Problem mit dem JSSON-Model hier ist, das diese Methode nur beim Laden der Daten aufgerufen wird
		// 				// und somit die Tols gesetzt werden
		// 				// springt der User zwischen den Ergebnissen hin und her  so werden nicht jedesmal die Daten nachgelesen
		// 				// und die letzten Tols sind gesetzt
		// 			}
		// 		}
		// 	});

...
], function (Controller, JSONModel, MessageToast, StringType, ValidateException, History) {
	"use strict";

	return Controller.extend("de.enercon.usbee.controller.recordresults_list.DetailSXN_list", {
		...
		onInit: function () {
			...
			oRouter.getRoute("Merkmal_SX_Quan_list").attachMatched(this._onRouteMatched, this);
		...
		onSave: function () {
			...
			if (oModel.hasPendingChanges()) {
				...
				// 	return;
				// }
				// var check_result2 = this.getOwnerComponent().checkbeforesavemess(this);
				// if (!check_result2) {
				...
				// //DokuPflicht
				// var check_result3 = this.getOwnerComponent().checkbeforesavedocu(this, "N");
				// if (!check_result3) {
			...
			// 	//var check_result = this._checkbeforesavesernr();
			// 	var oTable = this.getView().byId("idresuTableN");
			// 	check_result = this.getOwnerComponent().checkbeforesavesernr(this,oTable);
			// 	if (check_result) {
			// 		this.getOwnerComponent().onSave(this);
			// 	} else {
			// 		MessageToast.show(message2);
